<template>
  <!-- 添加电影的弹窗 -->
  <el-dialog title="添加电影咨询" v-model="addMovieDialogVisible">
    <el-form :model="newMovie" label-width="100px" :rules="rules">
      <el-form-item label="标题" prop="title" required>
        <el-input v-model="newMovie.title"></el-input>
      </el-form-item>
      <el-form-item label="来源" prop="source" required>
        <el-input v-model="newMovie.source"></el-input>
      </el-form-item>
      <el-form-item label="发布时间" required>
        <el-date-picker
          v-model="newMovie.releaseDate"
          type="date"
          placeholder="选择日期"
        ></el-date-picker>
      </el-form-item>
      <el-form-item label="简述" required>
        <el-input v-model="newMovie.description" type="textarea"></el-input>
      </el-form-item>
      <el-form-item label="资源链接" prop="url"  required>
        <el-input v-model="newMovie.url"></el-input>
      </el-form-item>
      <el-form-item label="咨询封面">
        <div class="avatar-uploader" @click="handleClick">
          <input
            type="file"
            ref="fileInput"
            @change="handleFileChange"
            style="display: none"
          />
          <img v-if="imageUrl" :src="imageUrl" class="avatar" />
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </div>
      </el-form-item>
    </el-form>

    <div slot="footer" class="dialog-footer">
      <el-button @click="addMovieDialogVisible = false">取消</el-button>
      <el-button type="primary" @click="addMovie">确定</el-button>
    </div>
  </el-dialog>
</template>
<script lang="ts" setup>
import { ref } from "vue";
import {
  handleError,
  ApiaddConsultation,
  ApichangeConstation,
} from "@/api/enter";
import { ElMessage } from "element-plus";
// 新电影表单
const newMovie = ref({
  image: "",
  title: "",
  description: "",
  source: "",
  releaseDate: "",
  url: "",
});
// 自定义验证函数，确保输入的是合法的URL格式
const validateUrl = (rule: any, value: string, callback: (arg0: Error | undefined) => void) => {
  const urlRegex =  /^www\.[^ "]+$/; // 正则表达式检查URL
  if (value && !urlRegex.test(value)) {
    callback(new Error('请输入有效的网页格式'));
  } else {
    callback(undefined);
  }
};

// 自定义验证函数：检查字段是否为空或仅包含空格
const validateNotEmpty = (rule: any, value: string, callback: (arg0: Error | undefined) => void) => {
  if (!value || value.trim() === '') {
    callback(new Error('此字段不能为空或全为空格'));
  } else {
    callback(undefined);
  }
};
const rules = ref({
  title: [
    { required: true, message: '请输入标题', trigger: 'blur' },
    { validator: validateNotEmpty, trigger: 'blur' }
  ],
  source: [
    { required: true, message: '请输入来源', trigger: 'blur' },
    { validator: validateNotEmpty, trigger: 'blur' }
  ],
  url: [
    { required: true, message: '请输入资源链接', trigger: 'blur' },
    { validator: validateUrl, trigger: 'blur' }
  ]
});
// 弹窗控制
const addMovieDialogVisible = ref(false);

// 打开添加电影的弹窗
const openAddMovieDialog = () => {
  addMovieDialogVisible.value = true;
};

// 添加咨询
const addMovie = async () => {
  // 去除开头和结尾的空格
  const trimmedTitle = newMovie.value.title
  const trimmedSource = newMovie.value.source
  if (trimmedTitle && trimmedSource && selectedFile.value) {
    // 提取年份
    if (newMovie.value.releaseDate) {
      const date = new Date(newMovie.value.releaseDate);
      // 获取年、月、日
      const year = date.getFullYear();
      const month = (date.getMonth() + 1).toString().padStart(2, "0"); // 月份从0开始，所以加1，并且保证是两位数
      const day = date.getDate().toString().padStart(2, "0"); // 确保日也是两位数
      // 组合成 yyyy-mm-dd 格式
      newMovie.value.releaseDate = `${year}-${month}-${day}`;
    } else {
      // 如果 newMovie.value.releaseDate 为空或无效，可以设置为默认值或进行错误处理
      newMovie.value.releaseDate = "0000-00-00"; // 例如，设置为 '0000-00-00'
    }
    try {
      // 首先添加电影信息
      await ApiaddConsultation(newMovie.value);
      if (selectedFile.value) {
        const formData = new FormData();
        formData.append("name", newMovie.value.title); // 电影名称
        formData.append("image", selectedFile.value);
        // 然后更改电影图片
        await ApichangeConstation(formData);
        ElMessage({
          message: "电影咨询添加成功",
          type: "success",
        });
        selectedFile.value = null;
        imageUrl.value = null;
      } else {
        ElMessage.error("请先选择图片");
      }
      addMovieDialogVisible.value = false;
      // 清空表单
      newMovie.value = {
        image: "",
        title: "",
        description: "",
        source: "",
        releaseDate: "",
        url: "",
      };
    } catch (error) {
      handleError(error);
    }
  } else {
    ElMessage.error("请填写完整的电影信息");
  }
};

const imageUrl = ref<string | null>("");
const fileInput = ref<HTMLInputElement | null>(null);
const selectedFile = ref<File | null>(null);
const handleAvatarSuccess = (file: File) => {
  imageUrl.value = URL.createObjectURL(file);
  selectedFile.value = file;
};

const beforeAvatarUpload = (file: File) => {
  const isJPG = file.type === "image/jpeg" || file.type === "image/png";
  const isLt2M = file.size / 1024 / 1024 < 2;
  if (!isJPG) {
    ElMessage.error("上传头像图片只能是 JPG 或 PNG 格式!");
  }
  if (!isLt2M) {
    ElMessage.error("上传头像图片大小不能超过 2MB!");
  }
  return isJPG && isLt2M;
};

const handleFileChange = (event: Event) => {
  const input = event.target as HTMLInputElement;
  if (input.files && input.files.length > 0) {
    const file = input.files[0];
    if (beforeAvatarUpload(file)) {
      handleAvatarSuccess(file);
    }
  }
};

const handleClick = () => {
  fileInput.value?.click(); // 打开文件选择对话框
};

defineExpose({ openAddMovieDialog });
</script>

<style scoped>
.dialog-footer {
  text-align: right;
}
.avatar-uploader {
  width: 220px;
  height: 150px;
  border: 1px dashed #d9d9d9;
  overflow: hidden;
  background-color: #e8e7e7;
  cursor: pointer;
  object-fit: cover;
}
.avatar-uploader img {
  width: 220px;
  height: 150px;
  object-fit: cover; /* 裁剪图片适应容器 */
}
</style>
